...
function generateFlower() {
  ...
  if (_shapeRoll < 0.05) {
    ...
    shapePool = [R.random_choice(['spiky','trap','trueHeart','sakura','tri','drop'])];
  ...
  const puffy    = R.random_bool(0.30);
  const minWidth = puffy ? 80 : 40;

  const _centerRoll = R.random_dec();
  const centerMode  = paletteName === 'pearl' ? (_centerRoll < 0.5 ? 'normal' : 'small') : (_centerRoll < 0.2 ? 'none' : _centerRoll < 0.6 ? 'normal' : 'small');
  ...
  for (let l = 0; l < numLayers; l++) {
    ...
    const _maxWidth = petals <= 7  ? 120 :
                      ...
                      petals <= 13 ? 80  :
                      petals <= 16 ? 65  : 55;
    let   petalWidth  = R.random_num(Math.min(minWidth, _maxWidth), _maxWidth) * WORLD_SCALE;
  ...
  if (showStamen) {
    ...
    const _bubblyShapes = ['oval','drop','trueHeart','sakura'];
    const _isBubbly = (shapePool.length === 2 && shapePool[0] === 'oval' && shapePool[1] === 'drop')
      || (shapePool.length === 1 && _bubblyShapes.includes(shapePool[0]));
    const _twirlsOk = numLayers <= 3 || _isBubbly;
    const stamenType = _stamenRoll < 0.405 ? 'og'
      : _stamenRoll < 0.64 ? 'sticks'
      : _stamenRoll < 0.74 ? 'messy'
      : _stamenRoll < 0.895 ? (_twirlsOk ? 'twirls' : 'og')
      : (centerMode !== 'none' ? 'zinnia' : 'og');
    ...
    if (stamenType === 'og') {
      ...
      for (let i = 0; i < numSticks; i++) {
        ...
        if (showPollen) {
          ...
          p.position.set(x2, y2, stamenZ + pollenR * 0.6);
          root.add(p);
        }
      }
      ...
      const antherGeo  = new THREE.IcosahedronBufferGeometry(1, 2);
      for (let i = 0; i < numSticks; i++) {
        const angle = (Math.PI * 2 / numSticks) * i;
        const x1 = Math.cos(angle) * stemInner, y1 = Math.sin(angle) * stemInner;
        const x2 = Math.cos(angle) * slen,      y2 = Math.sin(angle) * slen;
        ...
        anther.position.set(x2, y2, stamenZ + 0.01);
      ...
      const podGeo   = new THREE.IcosahedronBufferGeometry(1, 2);
  ...
  return { root, paletteName, numLayers, layerGroups, stamenAnim };
}
...
const { root, numLayers, layerGroups, paletteName, stamenAnim } = generateFlower();
