...
const FILTER_NORMAL = 'contrast(1.6)';
const FILTER_MONO   = 'contrast(1.55) saturate(0)';
...
const _noiseFShader = 'uniform sampler2D tDiffuse; uniform sampler2D tNoise; uniform float uStrengthBright; uniform float uStrengthDark; uniform float uContrast; uniform float uBrightness; uniform float uSpeckle; uniform float uSpeckleThr; varying vec2 vUv; vec3 linearToSRGB(vec3 c) { return pow(clamp(c, 0.0, 1.0), vec3(1.0/2.2)); } vec3 sRGBToLinear(vec3 c) { return pow(clamp(c, 0.0, 1.0), vec3(2.2)); } void main() { vec4 color = texture2D(tDiffuse, vUv); vec3 srgb = linearToSRGB(color.rgb); srgb = srgb * uBrightness; srgb = clamp((srgb - 0.5) * uContrast + 0.5, 0.0, 1.0); vec3 noise = texture2D(tNoise, vUv).rgb; vec3 invNoise = 1.0 - noise; srgb = 1.0 - (1.0 - srgb) * (1.0 - noise * uStrengthBright); srgb = srgb * (1.0 - invNoise * uStrengthDark + uStrengthDark); float luma = dot(srgb, vec3(0.299, 0.587, 0.114)); float darkMask = 1.0 - smoothstep(0.0, uSpeckleThr, luma); srgb = clamp(srgb + noise * uSpeckle * darkMask, 0.0, 1.0); color.rgb = sRGBToLinear(clamp(srgb, 0.0, 1.0)); gl_FragColor = vec4(color.rgb, 1.0); }';
